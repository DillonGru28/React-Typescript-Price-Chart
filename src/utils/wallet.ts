...
import { BASE_BSC_SCAN_URLS, CIDS } from 'config';
import { nodes } from './getRpcUrl';

const NETWORK_CONFIG = {
    [CIDS.MAINNET]: {
        name: 'BNB Smart Chain Mainnet',
        scanURL: BASE_BSC_SCAN_URLS[CIDS.MAINNET]
    },
    [CIDS.TESTNET]: {
        name: 'BNB Smart Chain Testnet',
        scanURL: BASE_BSC_SCAN_URLS[CIDS.TESTNET]
    }
};
...
export const setupNetwork = async (externalProvider?: ExternalProvider) => {
    ...
    const chainId = parseInt(process.env.REACT_APP_PUBLIC_CHAIN_ID, 10) as keyof typeof NETWORK_CONFIG;
